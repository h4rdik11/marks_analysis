<?php  
	...
	if(isset($data->new_pass) && isset($data->confirm_pass))
		$qry = $pdo_conn->prepare("UPDATE `admin` SET `firstname` = '".$data->fname."', `lastname` = '".$data->lname."', `phone` = '".$data->contact."',`emailid` = '".$data->email."', `pwd` = '".$data->new_pass."' WHERE `emp_id` = '".$id."'");
	else
		$qry = $pdo_conn->prepare("UPDATE `admin` SET `firstname` = '".$data->fname."', `lastname` = '".$data->lname."', `phone` = '".$data->contact."',`emailid` = '".$data->email."' WHERE `emp_id` = '".$id."'");
